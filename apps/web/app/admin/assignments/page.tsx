...
export default function AdminAssignmentsPage() {
  ...
  const authorized =
    hasPermission(session, "assessment.view") && hasPermission(session, "candidate.view");
  const isPlatformAdmin =
    session.session.organizationId === null &&
    (session.session.role === "SUPER_ADMIN" || session.session.role === "PLATFORM_ADMIN");
  const canManage =
    hasPermission(session, "assessment.manage") && hasPermission(session, "candidate.manage");
  ...
  async function loadBaseData() {
    ...
    try {
      const assessmentDefinitions = await listAssessmentDefinitions();
      setDefinitions(assessmentDefinitions);

      if (session.session.role === "ORGANIZATION_ADMIN") {
  ...
  async function refreshAssignments() {
    const currentOrganizationId = isPlatformAdmin ? organizationId.trim() : undefined;
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    if (isPlatformAdmin && !organizationId.trim()) {
    ...
    try {
      await createAdminAssessmentAssignment({
        ...(isPlatformAdmin ? { organizationId: organizationId.trim() } : {}),
        assessmentVersionId,
        userId: candidateId,
        maxAttempts,
  ...
  if (loading) {
    return (
      <AdminRoute permissions={["assessment.view", "candidate.view"]}>
      ...
      <div className="space-y-8">
        <header>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
            Assign published assessment versions to eligible students or employees. Assignment
            history and candidate attempts are preserved for auditability.
          </p>
        </header>
        ...
        {canManage ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            {isPlatformAdmin && !organizationId.trim() ? (
            ...
            <form onSubmit={handleCreate} className="mt-6 grid gap-5 lg:grid-cols-2">
              <label className="block">
                ...
                <select
                  ...
                  disabled={isPlatformAdmin && !organizationId.trim()}
              ...
              <label className="block">
                ...
                <select
                  ...
                  disabled={isPlatformAdmin && (!organizationId.trim() || candidates.length === 0)}
                  className="mt-2 min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-100"
                >
