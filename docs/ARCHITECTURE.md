...
## 58. R19.1B Administrative Operations Backend

The Control Centre backend exposes platform-scoped dashboard, administrator, role-template, permission, audit, organization, and global-user read models. Every growing directory uses deterministic `(created_at, id)` cursor pagination with bounded limits. Search is expressed through validated Prisma filters, and caller-provided organization IDs only narrow an already platform-authorized request. API projections explicitly select operational fields and exclude credential hashes, raw tokens, reset material, secrets, and unrestricted audit metadata.

Administrative invitation creation, resend, and revocation execute transactionally with the administrator mutation. Raw invitation tokens are hashed before persistence and are never returned by an administrative API. Each invitation has an observable `NotificationDelivery`, and each dispatch request has an `OutboxEvent` keyed by the invitation-token ID. If no approved email provider is configured, both records use `BLOCKED_CONFIGURATION`; this is an honest operational state, not a simulated delivery success. Revocation marks outstanding tokens revoked and cancels pending or configuration-blocked delivery work.

## 59. R19.1C Control Centre Security Boundary

The Control Centre consumes server-resolved effective permissions from the validated session response. This projection controls navigation and avoids unauthorized or unnecessary requests, but it never replaces API enforcement. Platform-global pages require a platform-scoped session and suppress their data-loading effects when that prerequisite is absent. Mutations use the shared CSRF-protected API client; authentication remains in HTTP-only cookies, 401 responses return users to a same-origin login path, and 403 responses are distinguished as access denial.

Responsive navigation preserves the accepted assessment, assignment and report-release workflows while adding platform dashboard, access governance, audit, organization and customer-support workspaces. Reusable dialogs provide keyboard focus containment and explicit impact confirmation for suspension, invitation revocation, role revocation and role deactivation. See `docs/R19_1C_CONTROL_CENTRE.md` for the launch route and invitation-delivery gate.
